...
function toast(msg,type='',dur=2600){
  ...
  clearTimeout(_tt);_tt=setTimeout(()=>e.classList.remove('show'),dur);
}
...
function confetti(n=55){
  ...
  for(let i=0;i<n;i++) setTimeout(()=>{
    ...
    e.style.cssText=`left:${10+Math.random()*80}%;top:0;width:${s}px;height:${s}px;`+
...
function goGame(id){
  const g=REGISTRY.find(x=>x.id===id);if(!g)return;
  document.getElementById('g-title').textContent=g.name;
  showScreen('game');
  const con=document.getElementById('g-container');con.innerHTML='';
  _curGame=g.init(con);
  _curGame.id=id;
}
...
function renderLobby(){
  ...
  REGISTRY.forEach(g=>{
    ...
    card.innerHTML=`${g.badge?`<span class="gcard-badge">${g.badge}</span>`:''}
...
function renderGlobalStats(){
  const all=loadStats();
  const body=document.getElementById('gs-body');body.innerHTML='';
  REGISTRY.forEach(g=>{
    const st=all[g.id];const wr=winRate(st);
    const d=document.createElement('div');d.className='gs-card';
    d.innerHTML=`<span class="gs-icon">${g.icon}</span>
      <div class="gs-info">
  ...
  clr.className='btn-d';clr.style.marginTop='14px';
